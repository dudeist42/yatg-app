import { Kbd, Modal, SearchField, Separator } from '@heroui/react';
import { useEffect, useState } from 'react';
import Link from 'next/link';
import { useDebounce } from 'use-debounce';
import { useInfiniteQuery } from '@tanstack/react-query';
import { queries } from '@/queries';
import { MovieSearchItem } from '../movie-search-item/movie-search-item';
...
export const SearchModal = ({ isOpen, onClose }: TSearchModalProps) => {
  const [searchQuery, setSearchQuery] = useState('');
  const [debouncedSearch] = useDebounce(searchQuery, 300);
  const { data: movies, isFetching } = useInfiniteQuery(
    queries.movie.find(debouncedSearch),
  );
