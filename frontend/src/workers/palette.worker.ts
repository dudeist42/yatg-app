...
const quantizeColors = (pixels: RGB[], bucketCount = 16): RGB[] => {
...
const findVibrant = (candidates: RGB[]): RGB => {
...
const findBrightest = (candidates: RGB[]): RGB =>
  candidates.reduce(
    (best, color) =>
      color.r + color.g + color.b > best.r + best.g + best.b ? color : best,
    candidates[0],
  );

const analyzePaletteFromBitmap = (bitmap: ImageBitmap): Palette => {
  ...
  if (pixels.length < MIN_PIXEL_COUNT) return DEFAULT_PALETTE;

  const topColors = quantizeColors(pixels, QUANTIZE_BUCKETS);
  if (topColors.length === 0) return DEFAULT_PALETTE;
  ...
  const dark = hslToRgb(
    dominantHsl.h,
    Math.min(dominantHsl.s * 1.2, 100),
    Math.max(dominantHsl.l * 0.15, 5),
  );
...
self.onmessage = ({ data: { type, imageBitmap, id } }: MessageEvent) => {
